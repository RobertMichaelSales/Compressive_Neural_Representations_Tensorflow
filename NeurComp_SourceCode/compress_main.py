...
from compress_utilities      import TrainStep,SignalToNoise,GetLearningRate,CalculatePointCloudDensity,CalculateStandardDeviation,LRStudy,BFStudy,MeanSquaredErrorMetric

#==============================================================================

def compress(network_config,runtime_config,training_config,metadata_config,i_filepath,o_filepath,index,checkpoint):
    ...
    # Get and display available memory (hard limit)
    available_memory = psutil.virtual_memory().available
    print("\n{:30}{:.3f} GigaBytes".format("Available Memory:",(available_memory/1e9)))
    
    # Set and display threshold memory (soft limit)
    ...
    # Check whether the input file size exceeds memory limits
    ...
    weights = DataClass(data_type="weights",is_tabular=metadata_config.is_tabular,exceeds_memory=exceeds_memory)
    weights.LoadData(input_data_path=i_filepath,columns=metadata_config.columns,shape=metadata_config.shape,dtype=metadata_config.dtype,normalise=metadata_config.normalise)
    ...
    output_directory = os.path.join(o_filepath,network_config.network_name + str(index))
    ...
    if runtime_config.data_generator_flag:
        dataset = MakeDatasetFromGenerator(volume=i_volume,values=i_values,weights=weights,batch_size=training_config.batch_size)
    else:
    ...
    if runtime_config.ensemble_flag and index:
        ...
        print("\n\n\n\nLoading weights!!!!!")
        SquashNet.set_weights(checkpoint)
        print("\n\n\n\nLoading weights!!!!!")
...
if __name__=="__main__":
    ...
    if (len(sys.argv) == 1):
        ...
        # Example: sys.argv[2] = '{"bf_study_flag": False,"graph_flag": False,"lr_study_flag": False,"stats_flag": False}'
        runtime_config = GenericConfigurationClass(json.loads(sys.argv[2]))
       
        # Example: sys.argv[3] = '{"batch_fraction": 0,"batch_size": 1024,"epochs": 30,"half_life": 3,"initial_lr": 0.005}'
    ...
    # metadata_config_dictionary = {"dtype": "float64","shape": (100520000,10),"columns": ([0,2,3],[8],[9]),"is_tabular": True,"normalise": True}   
    # metadata_config = GenericConfigurationClass(metadata_config_dictionary)
    # i_filepath = "/Data/Compression_Datasets/combustor_les_compressible_time/combustor_les_compressible_time_all_data.npy"
    # o_filepath = "/home/rms221/Documents/Compressive_Neural_Representations_Tensorflow/NeurComp_AuxFiles/outputs"
    
    
    
    metadata_config_dictionary = {"dtype": "float64","shape": (100000000,10),"columns": ([0,2,3],[8],[]),"is_tabular": True,"normalise": True}   
    metadata_config = GenericConfigurationClass(metadata_config_dictionary)
    i_filepath = "/Data/Compression_Datasets/combustor_les_compressible_time/combustor_les_compressible_time_all_data_interpolated.npy"
    o_filepath = "/home/rms221/Documents/Compressive_Neural_Representations_Tensorflow/NeurComp_AuxFiles/outputs"
    ...
    for index in range(1):
        checkpoint = compress(network_config=network_config,runtime_config=runtime_config,training_config=training_config,metadata_config=metadata_config,i_filepath=i_filepath,o_filepath=o_filepath,index=index,checkpoint=checkpoint)   
    ##
